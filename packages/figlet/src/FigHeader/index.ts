...
export function fromLine(line: string): FigletResult<FigHeader> {
  const splitLine = C.from(line.trim().split(" "))
...
function validatePrintDirection(
  ...
  return O.forEachF(E.Applicative)((value: string) =>
    E.chain_(headerFieldToInteger(value, "printDirection"), PrintDirection.fromValue)
  )(printDirection)
}
...
export const equalFigHeader: Equal<FigHeader> = Eq.makeEqual(Structural.equals)
